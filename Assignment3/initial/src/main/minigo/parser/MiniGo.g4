...
const_decl: CONST ID all_type? EQ expr sm_nl;
...
interface_decl:
	TYPE ID INTERFACE LCB (NL | interface_method)* RCB sm_nl;
...
expr6: expr6 (list_array_index | DOT_OP (ID | func_call)) | factor;
...
assignment_lhs: assignment_lhs (list_array_index | DOT_OP ID) | ID;
assign_op:
	EQ_ASSIGN_OP
	| ADD_ASSIGN_OP
	| SUB_ASSIGN_OP
...
call_stmt: (assignment_lhs DOT_OP |) ID LRB (expr_list |) RRB sm_nl;
return_stmt: RETURN (expr |) sm_nl;
// array struct interface type
array_type: list_array_index (prim_types | ID);

struct_type: ID STRUCT LCB (NL | struct_field)* RCB sm_nl?;
struct_field: ID all_type sm_nl;
// support
body: LCB stmt_list? RCB sm_nl*;
...
item: array_item | expr | LCB array_element RCB;
// structlit
structlit: ID LCB list_struct_element? RCB;
list_struct_element:
